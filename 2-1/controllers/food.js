...
const deleteFood = async (req,res,next) => {
    const food = Food.destroy({where:{name:req.params.name}})
    ...
    res.status(201).json(`${req.params.name} is deleted`)
}
...
const updateFood = async (req,res,next) => {
    ...
    const food = await Food.update(updatedFood,{where:{name:req.params.name}})
